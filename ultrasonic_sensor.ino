int echo_pin=10;//초음파센서 eco pin은 10번
int trig_pin=11;//초음파센서 trig pin은 11번
void setup() {
...
void loop() {
  ...
  //pulsln(핀번호, 핀상태)의 형태로 이루어져 있으며 핀 상태가 바뀌면
  //그때까지 경과한 시간을 ms(마이크로 세컨드)단위로 변환시켜줍니다.
  //단위는 마이크로 초입니다.

//  d의 속도는 초당 340미터를 이동하거나 29마이크로초 당 1센치를 이동합니다 
duration = pulseIn(echo_pin, HIGH);
  ...
  //high였을 때 시간(초음파가 보냈다 다시 들어온 시간)을 가지고 거리를 계산
        ...
        Serial.begin(distance);//시리얼 포트를 열어서 거리를 확인한다.
        Serial.begin("mm");//cm로 하고 싶으면 '/100'을 해주면 된다.
        delay(3);
             }  
